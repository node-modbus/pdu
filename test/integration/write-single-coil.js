...
describe("Write Single Coil", function () {
	it("should be [ address, value ] => [ address, value ]", function () {
		for (var i = 0; i < Help.trials; i++) {
			...
			var buf1  = Help.modbus.writeSingleCoil.request(addr, value);
			var buf2  = Help.addressValue(Help.modbus.writeSingleCoil.code, addr, [ new Buffer([ value ? 0xFF : 0x00, 0x00 ]) ]);
			var buf3  = Help.modbus.writeSingleCoil.response(addr, value);

			assert.deepEqual(buf1, buf2);
			assert.deepEqual({ address : addr, value : value }, Help.modbus.writeSingleCoil.parse(buf3.slice(1)));
		}
	});
});
