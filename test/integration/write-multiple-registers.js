...
describe("Write Multiple Registers", function () {
	it("should be [ start, end, blocks ] => [ start, end ]", function () {
		for (var i = 0; i < Help.trials; i++) {
			...
			var buf1   = Help.modbus.writeMultipleRegisters.request(start, end, blocks);
			var buf2   = Help.startEndValues(Help.modbus.writeMultipleRegisters.code, start, end, blocks);
			var buf3   = Help.modbus.writeMultipleRegisters.response(start, end);

			assert.deepEqual(buf1, buf2);
			assert.deepEqual({ start : start, end : end }, Help.modbus.writeMultipleRegisters.parse(buf3.slice(1)));
		}
	});
});
