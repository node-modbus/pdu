...
exports.Package   = function (fcode, data) {
	var buffer = Buffer.alloc(data.length + 1);
...
function load() {
	...
	exports.Request = function (buffer) {
		...
		throw new Error("Unknown request function");
	};
	...
	exports.Response = function (buffer) {
		...
		throw new Error("Unknown response function");
	};
}
...
function proxy(funct, method) {
	return function () {
		...
		var buffer = Buffer.alloc(stream.length + 1);
